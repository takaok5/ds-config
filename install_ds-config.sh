...
# ------------------- clone o update repo -------------------------------
if [ -d /opt/ds-config/.git ]; then
  echo "→ Repo già presente, aggiorno"
  git -C /opt/ds-config pull --rebase || die "git pull fallito"
else
  echo "→ Clono repo ds-config"
  git clone https://github.com/takaok5/ds-config.git /opt/ds-config \
    || die "git clone fallito"
fi
